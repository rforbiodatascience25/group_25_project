...
eur_sex_count <- data_metrics |>
  filter(`Genetic Ancestry Label` == "EUR") |>
  count(Sex)

axis_labels <- c(
  "Female" = paste0("Female (n = ", eur_sex_count$n[eur_sex_count$Sex == "Female"], ")"),
  "Male"   = paste0("Male (n = ", eur_sex_count$n[eur_sex_count$Sex == "Male"], ")")
)
...
## **Gene summary (Genomic Alterations & Instability)**
...
cor_matrix <- cor(gene_vars, use = "pairwise.complete.obs")

cor_df <- as.data.frame(cor_matrix) |>
  rownames_to_column("Var1") |>
  pivot_longer(-Var1, names_to = "Var2", values_to = "Correlation") |>
  mutate(
    Var1 = factor(Var1, levels = names(gene_vars)),
    Var2 = factor(Var2, levels = names(gene_vars))
  )
...
ggplot(cor_df, aes(Var1, Var2, fill = Correlation)) +
  ...
  labs(title = "Gene Variable Correlation Heatmap", x = NULL, y = NULL)
```
...
code_vars <- data_code |>
  select(
    ...
    `AJCC Lymph Node` = `Neoplasm Disease Lymph Node Stage American Joint Committee on Cancer Code`,
...
cor_df <- as.data.frame(cor_matrix) |>
  ...
  pivot_longer(-Var1, names_to = "Var2", values_to = "Correlation") |>
  mutate(
    Var1 = factor(Var1, levels = names(code_vars)),
    Var2 = factor(Var2, levels = names(code_vars))
  )
...
American Joint Committee on Cancer Metastasis Stage Code (MX) -\> Metastasis cannot be assessed or information is insufficient to assign an M category. Besides this always one class that has the greatest...
...
Not enough diversity in tumor types.
