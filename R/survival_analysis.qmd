...
#### Interaction between Hypoxia and TMB, and its effects in Overall Survival (Days)
...
data_heatmap <- data_gene |>
  ...
  mutate(
    TMB_Bin = case_when(
      ...
      `TMB (nonsynonymous)` <= median(`Mutation Count`, na.rm = TRUE) ~ "Low TMB")) |>
  ...
  ungroup()|>
  group_by(Hypoxia_Score_Type, TMB_Bin, Hypoxia_Bin) |>
...
ggplot(data_heatmap, aes(x = TMB_Bin, 
  ...
  theme(
    ...
    labs(
      ...
      subtitle = "Interaction between TMB and Hypoxia Score",
      x = "TMB (nonsynonymous)", 
      y = "Winter Hypoxia Score"
    )
```

From the previous analysis, a somewhat relevant correlation beteen TMB and hypoxia was rased. And to futher that analysis, these plots cheking the Interaction between All scores of Hypoxia and TMB, and its effects in Overall Survival (Days). The data clearly shows that Low TMB is the dominant and most favorable predictor for survival across all three hypoxia scoring methods.

The crucial finding is the sensitivity difference between the different scores. The Ragnum model identifies the most extreme outcomes, highlighting the best combination (Low TMB and Low Hypoxia, 826 days) and at the same time the worst survival scenario (High TMB, 330 days), making it the most predictive.

In contrast, the Buffa signature shows minimal difference between groups, and the Winter signature simply reinforces TMB's importance without hypoxia adding much predictive power. Therefore, while Low TMB is consistently beneficial, the overall conclusion about how much hypoxia contributes to survival hinges entirely on the specific calculation method (Ragnum, Winter, or Buffa) being used.
...
Hypoxia Scores are used to estimate how oxygen-deprived a tumor is based on gene expression. Higher scores indicate more severe hypoxia. Since hypoxic tumors tend to be more resistant to radiotherapy, these scores can be useful for understanding treatment response and patient outcomes. For this analysis, all three hypoxia scores wered used based on the diversity achieved in the previous analysis.
