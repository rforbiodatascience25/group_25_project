...
data_diag |> 
  ...
  count(Prior_Diagnosis = `Prior Diagnosis`,
    ...
    Survival = `Disease-specific Survival status (days)`) |> 
  group_by(Survival) |> 
...
#| fig-height: 6
bind_cols(data_diag |>
  select(Months_Disease = `Months of disease-specific survival (days)`,
    Survival_Status = `Disease-specific Survival status (days)`),
    data_gene |>
...
data_score |>
  ...
  select(`Patient ID`,
    `Disease-specific Survival status (days)`),
    by = "Patient ID") |>
  filter(`Disease-specific Survival status (days)` == "1:DEAD WITH TUMOR") |>
  mutate(across(c(`Buffa Hypoxia Score`,
    `Ragnum Hypoxia Score`,
    `MSI MANTIS Score`,
    `MSIsensor Score`),
