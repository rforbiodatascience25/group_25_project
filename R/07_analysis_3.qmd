...
### Average biological scores (Aneuploidy, hypoxia, MSI) Across Age Groups Divided by Sex

André Comment - \> values different scales so heatmap coloring is not valuable.
...
data_score |>
  ...
  geom_text(aes(label = round(Mean_Score, 2)), color = "black", size=2) +
...
Comment André: This should go to describe as well

Undestand the ideia but for starters we have other codying systems and lables are weird
