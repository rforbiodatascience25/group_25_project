...
# Difference between genomic instability measures in 
data_gene_diag <- data_gene_diag |>
...
# Change data format to add all graphs together easily
data_gene_diag_long <- data_gene_diag |>
  pivot_longer(cols = c(
      `Mutation Count`,
      `TMB (nonsynonymous)`,
...
data_gene_diag_long |>
  ...
  labs(title = "Genomic Instability Measures by Tumor Type")
...
[Conclusion]{.underline}: Not enough disersity in tumor types, but other subtypes of adenocarcinoma have a higher fraction of the genome altered compared to others. From now on discart all tumor types that are not pancreas adenocarcinoma (ductal type or other subtype)
