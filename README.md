# group_25_project

Group 25 Project on R for Bio Data Science 2025

Project Contributors:

André Godinho, s253707, Msc Bioinformatics, git: andregodinhodtu

Catarina Anastácio, s253709, Msc Bioinformatics  git: CatarinaAnastacio

Lily Whipple, s243865, Msc Bioinformatics  git: LilyW08

Szymon Cholewiński, s253711, Msc Bioinformatics  git: InfiniteLobster

Magnus Strange, s190867, Msc Bioinformatics  git: s190867

## Quarto Presentation

https://raw.githack.com/rforbiodatascience25/group_25_project/main/doc/presentation.html

##  Original Data: Pancreatic Adenocarcinoma TCGA PanCancer data

https://www.cbioportal.org/study/summary?id=paad_tcga_pan_can_atlas_2018

PAAD TCGA Pan-Cancer Atlas (2018) contains pancreatic adenocarcinoma patient data including demographics (age, sex, race, ancestry), genomic instability metrics (mutation count, TMB, fraction genome altered), biological scores (aneuploidy, hypoxia, MSI), cancer staging/clinical codes, and survival outcomes with therapy history.​

## OneDrive Data

https://dtudk-my.sharepoint.com/:f:/g/personal/s253711_dtu_dk/EnUhq8OXAIRLiCfK5HAfa_IBmmXm8v_PSah1HKNBuqAerw?e=00m5vs

